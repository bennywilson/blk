# blk\_engine

Download and install:

1. Run setup.bat (downloads gaussian_splats. ~6.6GB )
2. FBX SDK 2020.3 https://aps.autodesk.com/developer/overview/fbx-sdk
3. Vulkan 1.4 https://www.lunarg.com/vulkan-sdk/
4. DXC Compiler and add binary to path: https://github.com/microsoft/DirectXShaderCompiler/releases
5. Run setup.py via python setup.py


References and Links

gaussian-splatting - "the official authors implementation associated with the paper "3D Gaussian Splatting for Real-Time Radiance Field Rendering"
https://github.com/graphdeco-inria/gaussian-splatting

vk_gaussian_splatting - "Nvidia's testbed to explore and compare various approaches to real-time visualization of 3D Gaussian Splatting (3DGS)""
https://github.com/nvpro-samples/vk_gaussian_splatting

DiverseShot.AI - .ply 3DGS datasets
https://www.diverseshot.com/blog/3d-gaussian-splatting-flies-examples

...
void CannonBallUIComponent::enable_internal(const bool bEnable) {
	...
	m_CannonBallActivatedStartTime = -1.0f;
	if (bEnable) {
		m_pSparkModel = GetOwner()->component(3)->GetAs<StaticModelComponent>();
		m_pBoomModel = GetOwner()->component(4)->GetAs<StaticModelComponent>();
		m_pSmokeModel = GetOwner()->component(5)->GetAs<StaticModelComponent>();
...
void CannonBallPauseMenuUIComponent::enable_internal(const bool bEnable) {
	...
	if (bEnable) {
		...
		for (int i = 0; i < m_Widgets.size(); i++) {
			m_Entity.add_component(&m_Widgets[i]);
		...
		for (int i = 0; i < m_SliderWidgets.size(); i++) {
			m_Entity.add_component(&m_SliderWidgets[i]);
		...
		for (int i = 0; i < m_Widgets.size(); i++) {
			m_Entity.remove_component(&m_Widgets[i]);
			m_Widgets[i].Enable(false);
			m_Widgets[i].UnregisterEventListener(this);
		}

		for (int i = 0; i < m_SliderWidgets.size(); i++) {
			m_Entity.remove_component(&m_SliderWidgets[i]);
			m_SliderWidgets[i].Enable(false);
...
void CannonBallGameSettingsComponent::SaveSettings() {
	...
	kbFile outFile;
	outFile.Open("./settings/game_settings.txt", kbFile::FT_Write);

	GameEntity gameSettingsEnt;
	gameSettingsEnt.add_component(this);
	outFile.WriteGameEntity(&gameSettingsEnt);
	outFile.Close();
	gameSettingsEnt.remove_component(this);
}
...
CannonBallGameSettingsComponent* CannonBallGameSettingsComponent::Get() {
	...
	if (gameSettingsFile.Open("./settings/game_settings.txt", kbFile::FT_Read)) {
		GameEntity* const gameEntity = gameSettingsFile.ReadGameEntity();
		s_pInstance = (CannonBallGameSettingsComponent*)gameEntity->component<CannonBallGameSettingsComponent>();

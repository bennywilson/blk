...
private:
	// Data
	GameEntityPtr	m_CannonBallImpactFX;
	GameEntityPtr	m_ShakeNBakeFX;
	GameEntityPtr	m_SplashFX;
	...
	// Run time
	GameEntityPtr	m_HeadBand;
	GameEntityPtr	m_HeadBandInstance[2];
...
protected:
	...
	bool CheckForBlocker(const Vec3 moveVec) {
		...
		for (int i = 0; i < g_pCannonGame->GetGameEntities().size(); i++) {
			...
			KungFuSnolafComponent* const pSnolaf = pGameEnt->component<KungFuSnolafComponent>();

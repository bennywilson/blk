...
public:
	...
	virtual void BeginState_Internal(T prevState) override {
		kbFlingPhysicsComponent* const pFlingPhysics = this->GetSheep()->component<kbFlingPhysicsComponent>();
...
public:
	...
	virtual void BeginState_Internal(T prevState) override {
		...
		kbString cur, next;
		if (this->m_pActorComponent->GetOwner()->component<SkeletalModelComponent>()->GetCurAnimationName() != nullptr) {
			cur = *this->m_pActorComponent->GetOwner()->component<SkeletalModelComponent>()->GetCurAnimationName();
		}

		if (this->m_pActorComponent->GetOwner()->component<SkeletalModelComponent>()->GetNextAnimationName() != nullptr) {
			next = *this->m_pActorComponent->GetOwner()->component<SkeletalModelComponent>()->GetNextAnimationName();
		}
...
void KungFuSheepComponent::enable_internal(const bool bEnable) {
	...
	if (bEnable) {
		...
		if (GetOwner()->is_prefab() == false) {
			m_HeadBandInstance[0].SetEntity(g_pGame->CreateEntity(m_HeadBand.GetEntity()));
			m_HeadBandInstance[0].GetEntity()->set_position(owner_position());
			m_HeadBandInstance[0].GetEntity()->set_rotation(owner_rotation());

			m_HeadBandInstance[1].SetEntity(g_pGame->CreateEntity(m_HeadBand.GetEntity()));
			m_HeadBandInstance[1].GetEntity()->set_position(owner_position());
			m_HeadBandInstance[1].GetEntity()->set_rotation(owner_rotation());
		}
...
void KungFuSheepComponent::OnAnimEvent(const kbAnimEventInfo_t& animEventInfo) {
	...
	if (animEventName == CannonBallImpact) {
		...
		if (m_CannonBallImpactFX.GetEntity() != nullptr) {
			GameEntity* const pCannonBallImpact = g_pGame->CreateEntity(m_CannonBallImpactFX.GetEntity());
...
void KungFuSheepComponent::update_internal(const float DT) {
	...
	if (m_SkelModelsList[0]->GetBoneWorldMatrix(kbString("Head"), boneMat)) {
		...
		m_HeadBandInstance[0].GetEntity()->set_position(boneMat.GetOrigin() + axis1 * 0.1f - axis3 * 0.15f);
		m_HeadBandInstance[1].GetEntity()->set_position(boneMat.GetOrigin() + axis1 * 0.1f + axis2 * 0.01f - axis3 * 0.15f);
	}
	...
	kbClothComponent* const pCloth1 = m_HeadBandInstance[0].GetEntity()->component<kbClothComponent>();
	pCloth1->SetClothCollisionSphere(0, collisionSphere);

	kbClothComponent* const pCloth2 = m_HeadBandInstance[1].GetEntity()->component<kbClothComponent>();
	...
	if (g_CannonBallTest.GetBool() == false && m_health > 0.0f && IsCannonBalling() == false) {
		...
		for (int i = 0; i < g_pCannonGame->GetGameEntities().size(); i++) {
			...
			GameEntity* const pEnt = g_pCannonGame->GetGameEntities()[i];
...
void KungFuSheepComponent::PlayShakeNBakeFX() {
	...
	GameEntity* const pShakeNBakeFX = g_pGame->CreateEntity(m_ShakeNBakeFX.GetEntity());
	pShakeNBakeFX->set_position(owner_position());
	pShakeNBakeFX->set_rotation(owner_rotation());
...
void KungFuSheepComponent::PlayCannonBallFX(const Vec3 location) {
	...
	GameEntity* const pCannonBallImpact = g_pGame->CreateEntity(m_CannonBallImpactFX.GetEntity());
	pCannonBallImpact->set_position(location);
	pCannonBallImpact->set_rotation(owner_rotation());
	pCannonBallImpact->DeleteWhenComponentsAreInactive(true);
...
void KungFuSheepComponent::EnableHeadBand(const bool bEnable) {
	...
	if (m_HeadBandInstance[0].GetEntity() != nullptr) {
		m_HeadBandInstance[0].GetEntity()->component<SkeletalModelComponent>()->Enable(bEnable);
		m_HeadBandInstance[1].GetEntity()->component<SkeletalModelComponent>()->Enable(bEnable);
	}
}

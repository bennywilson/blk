...
public:
	...
	virtual void UpdateState_Internal() override {
		...
		for (int i = 0; i < g_pCannonGame->GetGameEntities().size(); i++) {
			...
			GameEntity* const pGameEnt = g_pCannonGame->GetGameEntities()[i];
			CannonActorComponent* const pTargetActor = pGameEnt->component<CannonActorComponent>();
...
public:
	...
	virtual void BeginState_Internal(T) override {
		...
		GameEntity* const pOwner = this->GetSnolaf()->GetOwner();
		for (int i = 0; i < pOwner->num_components(); i++) {
			ParticleComponent* const pParticle = pOwner->component(i)->GetAs<ParticleComponent>();
		...
		Mat4 worldMatrix;
		this->m_pActorComponent->GetOwner()->calculate_world_matrix(worldMatrix);
		...
		if (m_DeathSelection == 0) {
			...
			GameEntity* const pOwner = this->m_pActorComponent->GetOwner();
			for (int i = 0; i < pOwner->num_components(); i++) {
				SkeletalModelComponent* const pSkelModelComp = pOwner->component(i)->GetAs<SkeletalModelComponent>();
	...
	void UpdateFlyingDeath(const float dt) {
		...
		SkeletalModelComponent* const pSnolafComp = pOwner->component<SkeletalModelComponent>();
		...
		if (pSnolafComp->GetBoneWorldPosition(spine3BoneName, spine3WorldPos)) {
			...
			pOwner->set_position(m_OwnerPosOverride + vecOffset);
		}
...
void KungFuSnolafComponent::enable_internal(const bool bEnable) {
	...
	if (bEnable) {
		...
		for (int i = 0; i < GetOwner()->num_components(); i++) {
			if (GetOwner()->component(i)->IsA(ParticleComponent::GetType()) == false) {
				continue;
			}

			ParticleComponent* const pParticle = (ParticleComponent*)GetOwner()->component(i);

			if (pParticle->name() == SmallLoveHearts.stl_str()) {
				m_pSmallLoveHearts = pParticle;
				m_pSmallLoveHearts->enable_new_spawns(false);
			} else if (pParticle->name() == LargeLoveHearts.stl_str()) {
...
void KungFuSnolafComponent::OnAnimEvent(const kbAnimEventInfo_t& animEventInfo) {
	...
	if (animEvent.GetEventName() == LeftFootStep || animEvent.GetEventName() == RightFootStep) {
		if (m_FootStepImpactFX.GetEntity() != nullptr) {
			GameEntity* const pFootStepFX = g_pGame->CreateEntity(m_FootStepImpactFX.GetEntity());
			...
			pFootStepFX->set_position(decalPosition);
...
void KungFuSnolafComponent::update_internal(const float DT) {
	...
	if (m_CurrentState == KungFuSnolafState::Hug) {
		...
		kbDeleteEntityComponent* const pDeleteComp = component<kbDeleteEntityComponent>();
...
void KungFuSnolafComponent::ResetFromPool() {
	...
	for (int i = 0; i < GetOwner()->num_components(); i++) {
		kbFlingPhysicsComponent* const pFling = GetOwner()->component(i)->GetAs<kbFlingPhysicsComponent>();
		if (pFling != nullptr) {
			pFling->Enable(false);
			continue;
		}

		GetOwner()->component(i)->Enable(false);
		GetOwner()->component(i)->Enable(true);
	}
...
void KungFuSnolafComponent::DoPoofDeath() {
	...
	GameEntity* const pCannonBallImpact = g_pGame->CreateEntity(m_PoofDeathFX.GetEntity());
	pCannonBallImpact->set_position(owner_position());
	pCannonBallImpact->set_rotation(owner_rotation());
...
void KungFuSnolafComponent::SpawnAndFlingDecapHead() {
	...
	GameEntity* const pDecapHead = g_pGame->CreateEntity(m_DecapitatedHead.GetEntity());
	const Vec3 headPos = owner_position() + Vec3(0.0f, 1.75f, 0.0f);
	pDecapHead->set_position(headPos);
	pDecapHead->set_rotation(owner_rotation());
	pDecapHead->DeleteWhenComponentsAreInactive(true);

	kbFlingPhysicsComponent* const pFlingComp = pDecapHead->component<kbFlingPhysicsComponent>();
...
void KungFuSnolafComponent::SpawnAndFlingTopAndBottomHalf() {
	...
	GameEntity* const pTopHalf = g_pGame->CreateEntity(m_TopHalfOfBody.GetEntity());
	const Vec3 topPos = owner_position() + Vec3(0.0f, 1.0f, 0.0f);
	pTopHalf->set_position(topPos);
	pTopHalf->set_rotation(owner_rotation());
	pTopHalf->DeleteWhenComponentsAreInactive(true);

	kbFlingPhysicsComponent* const pTopFlingComp = pTopHalf->component<kbFlingPhysicsComponent>();
	if (pTopFlingComp != nullptr) {
		pTopFlingComp->Enable(false);
		pTopFlingComp->Enable(true);
	}

	GameEntity* const pBottomHalf = g_pGame->CreateEntity(m_BottomHalfOfBody.GetEntity());
	const Vec3 bottomPos = owner_position() + Vec3(0.0f, 0.2f, 0.0f);
	pBottomHalf->set_position(bottomPos);
	pBottomHalf->set_rotation(owner_rotation());
	pBottomHalf->DeleteWhenComponentsAreInactive(true);

	kbFlingPhysicsComponent* const pBottomFlingComp = pBottomHalf->component<kbFlingPhysicsComponent>();
...
void KungFuSnolafComponent::SpawnSplash() {
	...
	GameEntity* const pSplash = g_pGame->CreateEntity(m_SplashFX.GetEntity());
	pSplash->set_position(owner_position());

...
void kbClothComponent::update_internal(const float dt) {
	...
	for (int i = 0; i < GetOwner()->num_components(); i++) {
		if (GetOwner()->component(i)->IsA(SkeletalModelComponent::GetType())) {
			pSkelRenderComponent = static_cast<SkeletalModelComponent*>(GetOwner()->component(i));
...
void kbClothComponent::RunSimulation(const float inDeltaTime) {
	...
	for (int massIdx = 0; massIdx < m_Masses.size(); massIdx++) {
		...
		Vec3 newLocation = m_Masses[massIdx].position();// + ( totalForce * DeltaTime );
		...
		Vec3 velocity = m_Masses[massIdx].position() - m_Masses[massIdx].m_LastPosition;
		m_Masses[massIdx].m_LastPosition = m_Masses[massIdx].position();

		newLocation += velocity * (1.0f - g_ClothFriction.GetFloat()) + totalForce * (DeltaTime * DeltaTime);
		m_Masses[massIdx].set_position(newLocation);
		m_Masses[massIdx].m_FrameForces = Vec3::zero;
	}
	...
	for (int iIteration = 0; iIteration < m_NumConstrainIterations; iIteration++) {
		...
		for (int iSpring = 0; iSpring < m_Springs.size(); iSpring++) {
			...
			const Vec3 mass1Pos = m_Masses[mass1Idx].position();
			const Vec3 mass2Pos = m_Masses[mass2Idx].position();
			...
			if (m_Masses[mass1Idx].m_bAnchored == false) {
				m_Masses[mass1Idx].set_position(mass1Pos + (finalOffset * 0.5f));
			}

			if (m_Masses[mass2Idx].m_bAnchored == false) {
				m_Masses[mass2Idx].set_position(mass2Pos - (finalOffset * 0.5f));
			}
		}
	...
	for (size_t massIdx = 0; massIdx < m_BoneInfo.size(); massIdx++) {
		...
		if (curX < (size_t)m_Width - 1) {
			xAxis = (m_Masses[massIdx + 1].position() - m_Masses[massIdx].position()).normalize_safe();
		} else {
			xAxis = (m_Masses[massIdx].position() - m_Masses[massIdx - 1].position()).normalize_safe();
		}

		if (curY > 0) {
			yAxis = (m_Masses[massIdx - m_Width].position() - m_Masses[massIdx].position()).normalize_safe();
		} else {
			yAxis = (m_Masses[massIdx].position() - m_Masses[massIdx + m_Width].position()).normalize_safe();
		}
...
void kbClothComponent::SetupCloth() {
	...
	scaleMatrix[0].x = GetOwner()->scale().x;
	scaleMatrix[1].y = GetOwner()->scale().y;
	scaleMatrix[2].z = GetOwner()->scale().z;
	...
	for (int i = 0; i < m_Masses.size(); i++) {
		...
		m_Masses[i].m_LastPosition = m_Masses[i].position();

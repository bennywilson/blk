...
void kbPropertiesTab::EventCB(const widgetCBObject* const widgetCBObject) {
	...
	{
		m_SelectedEntities.clear();
		m_pTempPrefabEntity = new kbEditorEntity(const_cast<GameEntity*>(g_Editor->GetCurrentlySelectedPrefab()->GetGameEntity(0)));
		RefreshEntity();
	}
	break;
	}
...
void kbPropertiesTab::CheckButtonCB(Fl_Widget* widget, void* voidPtr) {
	...
	/*
		GameEntity *const pGameEntity = (GameEntity*)( pModifiedComponent->IsA( kbGameComponent::GetType() ) ? ( pModifiedComponent->GetOwner() ) : ( nullptr ) );

		if ( pGameEntity != nullptr && pGameEntity->component(0) == pModifiedComponent ) {
			// Refresh all components if the transform component was modified
			TransformComponent *const pTransformComponent = (TransformComponent*)pGameEntity->component(0);

			for ( int i = 0; i < pGameEntity->num_components(); i++ ) {
				kbComponent *const pCurComp = pGameEntity->component(i);
	...
	*((bool*)userData->m_pVariablePtr) = (bool)buttonVal;
	...
	if (pModifiedComponent->IsA(TransformComponent::GetType())) {
...
void kbPropertiesTab::ClearPointerButtonCB(Fl_Widget* widget, void* voidPtr) {
	...
	if (userData->m_VariableType == KBTYPEINFO_GAMEENTITY) {
		const kbPrefab* const pPrefab = g_Editor->GetCurrentlySelectedPrefab();
		GameEntityPtr& pEntityPtr = userData->m_GameEntityPtr;
...
void kbPropertiesTab::ArrayExpandCB(Fl_Widget* widet, void* userData) {
	...
	PropertyChangedCB(GameEntityPtr());
}
...
void kbPropertiesTab::PropertyChangedCB(const GameEntityPtr entityPtr) {
...
void kbPropertiesTab::RefreshComponent(kbEditorEntity* const pEntity, kbComponent* const pComponent, kbComponent* const pParentComponent, int& startX, int& curY, const int inputHeight, const bool bIsStruct, const void* const pArrayPtr, const int arrayIndex) {
	...
	// Delete button
	if (pComponent->IsA(TransformComponent::GetType()) == false) {
		if (bIsStruct == false) {
...
void kbPropertiesTab::RefreshEntity() {
	...
	if (m_SelectedEntities.size() == 1 || m_pTempPrefabEntity != nullptr) {	// todo: Don't display properties if multiple entities are selected
		...
		const GameEntity* pGameEntity = pEntity->GetGameEntity();

		for (size_t i = 0; i < pGameEntity->num_components(); i++) {
			RefreshComponent(pEntity, const_cast<kbGameComponent*>(pGameEntity->component(i)), nullptr, startX, curY, inputHeight);
			curY += LineSpacing();
		}
	}
...
void kbPropertiesTab::RefreshProperty(
	...
	{
		GameEntityPtr* const pEntityPtr = (GameEntityPtr*)byteOffsetToVar;
		cbData.m_pVariablePtr = pEntityPtr;
		GameEntity* const pEntity = pEntityPtr->GetEntity();
		if (pEntity == nullptr) {
			...
			Fl_Text_Display* propertyNameLabel = new Fl_Text_Display(xPos + propertyNamePixelWidth, yPos, 0, inputHeight, pEntity->name().c_str());
			propertyNameLabel->textsize(FontSize());
			propertyNameLabel->align(FL_ALIGN_RIGHT);
		}
...
void kbPropertiesTab::DeleteComponent(Fl_Widget* widget, void* voidptr) {
	...
	int componentIdx = -1;
	GameEntity* const pEntity = (GameEntity*)componentToDelete->GetOwner();	// ENTITY HACK
	for (componentIdx = 0; componentIdx < pEntity->num_components(); componentIdx++) {
		if (pEntity->component(componentIdx) == componentToDelete) {
			break;
		}
	}

	pEntity->remove_component(componentToDelete);

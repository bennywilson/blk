...
void kbResourceTab::EntitySelectedCB(Fl_Widget* pWidget, void* pUserData) {
	...
	if (Fl::event_button() == FL_LEFT_MOUSE) {
		...
		const std::string DeleteEntity = "Delete entity " + g_pResourceTab->m_EntityList[selectedItemIndex].m_pEntity->GetGameEntity()->name().stl_str();
		const std::string ZoomToEntity = "Zoom to entity " + g_pResourceTab->m_EntityList[selectedItemIndex].m_pEntity->GetGameEntity()->name().stl_str();
...
void kbResourceTab::ZoomToEntityCB(Fl_Widget* pWidget, void* pUserData) {
	...
	Vec3 vecTo = (camPos - pEditorEntity->position());
	vecTo.y = 0;

...
struct SceneData {
	...
	// texture_list: the (SceneData)base_instance cast below assigns
	// element-wise down BaseData's flattened scalar stream rather than
	// reinterpreting bytes, so every member here lines up with the matching
	// tightly-packed field of the C++ SceneInstanceData -- entity_id at
	// offset 304, right after texture_list's 16 floats.
	float4 entity_id;
};
